...
EFI_STATUS SaveMemoryMap(struct MemoryMap *map, EFI_FILE_PROTOCOL *file) {
  ...
  for (iter = (EFI_PHYSICAL_ADDRESS)map->buffer, i = 0;
    ...
    len = AsciiSPrint(buf, sizeof(buf), "%u, %x, %-ls, %081x, %lx, %lx\n", i,
...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE image_handle,
  ...
  Print(L"Kernel: 0x%01x - 0x%01x (%lu bytes)\n", kernel_first_addr,
        kernel_last_addr, kernel_file_size);

  status = gBS->FreePool(kernel_buffer);
  if (EFI_ERROR(status)) {
    Print(L"faild to free pool: %r", status);
    Halt();
  }
  ...
  if (status == EFI_SUCCESS) {
    ...
    if (EFI_ERROR(status)) {
      Print(L"failed to open read volume file: %r\n", status);
      Halt();
    }
  } else {
    EFI_BLOCK_IO_PROTOCOL *block_io;
