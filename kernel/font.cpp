...
void WriteString(PixelWriter *writer, const FrameBufferConfig *config, int x, int y, const PixelColor &color, const char *str) {
  for (int i = 0; str[i] != '\0'; ++i) {
    WriteAscii(writer, config, x + i * 8, y, color, str[i]);
  }
}
